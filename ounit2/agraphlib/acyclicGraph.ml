...
module type Point = sig
  ...
  module Constraint : Stdlib.Set.S with type elt = (t * constraint_type * t)
...
module Make (Point:Point) = struct
  ...
  *)
  ...
  struct
    ...
    let repr n t = Map.find n t.tab_fwd
  ...
  let pmap_modify k f m = PMap.update k (Option.map f) m
  ...
  let rec repr g u =
    ...
    | exception Not_found ->
      failwith
        ("Universe " ^ Point.pr (Index.repr u g.table) ^" undefined.")
